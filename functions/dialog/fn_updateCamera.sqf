...
_target attachto [_preview,_targetPos,""];
...
if ((getposasl _cam select 2) < (getposasl _preview select 2)) then {
    _disCoef = ((getposasl _target select 2) - (getposasl _preview select 2)) / ((getposasl _target select 2) - (getposasl _cam select 2) + 0.001);
